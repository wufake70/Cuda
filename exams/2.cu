/*
2.利用GPU找出一列包含1000000个元素的数组的最大的5个值.
...
__device__ __host__ void sort(int *array, int k, int data)
{
    for(int i=0; i<k; i++)
    {
        if(array[i] == data)
...
__global__ void gpu_topk(int* a,int* b,int length,int topk)
{
    // 每个线程维护一个数组
