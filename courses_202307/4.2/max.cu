...
__global__ void sum_gpu(int* a,int n,int* result)
{
    ...
    for(int i=blockIdx.x*blockDim.x+threadIdx.x;i<n;i+=BLOCK_SIZE*GRID_SIZE){
        // tmp+=a[i];
        // result是全局变量，多线程访问时需要原子操作
        // tmp=atomicMax(result,a[i]);
        atomicMax(result,a[i]);
    }
    // 载入 shared memory并等待完成
    // share_memory[threadIdx.x]=tmp;
    share_memory[threadIdx.x]=result[0];
    __syncthreads();
    ...
    for(int total_threads=BLOCK_SIZE/2;total_threads>=1;total_threads/=2){
        ...
        if(threadIdx.x<total_threads){
            ...
            if(share_memory[threadIdx.x]<share_memory[threadIdx.x+total_threads]){
                share_memory[threadIdx.x]=share_memory[threadIdx.x+total_threads];
            }
        }
        __syncthreads();
    }
    ...
    if(blockIdx.x*blockDim.x<n){
        if(threadIdx.x==0){
            ...
            // 原子操作
            atomicMax(result,share_memory[0]);
